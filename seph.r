...
seph.counts <- all.counts[, grepl("SepH", colnames(all.counts))];

### edgeR ###

counts <- seph.counts;

group <- factor(rep(c("S22", "SW14", "SW22" ), each = 2))
...
gr2coef <- c(2, 3);
names(gr2coef) <- c("SW14", "SW22");
fit <- glmQLFit(y, design)


# {{{ list of data frames of top tags.
tops_list <- list();
for(gr in c("SW14", "SW22")) {
...
for(iddo in rownames(sig_ord)) {
  ...
  if((!exists("in_gene")) | (abs(in_gene$distance) > pg_thresh) ) {
...
postprocess <- c(
"cd aarout",
"perl ../code/csv_merge.pl -outfile seph.csv -- SW14.csv SW22.csv",
"cp seph.csv ~/mnt/wstemp/",
""
);
...
# coef2bedgraph(fico, 2, "../bedgraph/sw14_fico.bedgraph");
# coef2bedgraph(fico, 3, "../bedgraph/sw22_fico.bedgraph");
