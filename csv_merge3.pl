...
my @head = (qw(position), @noex,  qw(minFDR closestgene lgene
...
my $outCnt = 0;
### @all is like [$noex, $pos, $lndiff, $fdr]
...
else { @iout = split("", "-" x $infnCnt); }


tablist($pos, @lndiff, $rr->[3], $mindisthr->{id},
@lout, @iout, @rout
);
