...
rm BldC_vnz_sco_regulon_overlap2.csv
perl code/vnz_sco_bldc_regulon2.pl -outfile BldC_vnz_sco_regulon_overlap2.csv \
-scofn BldCChIP-chipSCONatureCommunications.csv -vnzfn BldCRegulon_lndiffFinalTable.csv
cp BldC_vnz_sco_regulon_overlap2.csv ~/mnt/wstemp/matt/
...
if($lgene) {
$left{$lgene} = [@ll[5,6]];
$sco{$lgene} = ["left", @ll[2, 5, 6]];
}
if($rgene) {
$right{$rgene} = [@ll[9,10]];
$sco{$rgene} = ["right", @ll[2, 9, 10]];
}
if($igene) {
$in{$igene} = [@ll[13,14]];
$sco{$igene} = ["in", @ll[2, 13, 14]];
}
$lineCnt += 1;
...
if($skip) {
  for (1..$skip) {
    ...
    my @head = split(/\t/, $head);
    if($wantlog) {
      splice(@head, 1, 3, map {"log2_" . $_} @head[1..3])
    }
    tablist(@head[0..4], "vnzgene", "scogene", "sco_lfc", "sco_distfrompeak");
  }
}
# local $/ = ""; # For reading multiline records separated by blank lines.
my @done;
...
for my $vnzgene ($lgene, $igene, $rgene) {
  if($vnzgene) {
    if(grep {$_ eq $vnzgene} @done) { next; }
    my @out = @ll[0..4];
    my @log2 = log2(@ll[1..3]);
    if($wantlog) {
    splice(@out, 1, 3, @log2);
    ...
    if(exists($scortho{$vnzgene})) {
      ...
      if(exists($sco{$scogene})) {
        ...
        if(
           ($scoref->[0] eq 'left' and $scoref->[2] == -1)
        or ($scoref->[0] eq 'right' and $scoref->[2] == 1)
        or ($scoref->[0] eq 'in') ) {
        push(@out, @{$scoref}[1,3]);
        }
      }
